 ...
 */
function requestBannerAd(requestId, options) {
  ...
  return _NativeCachedBannerModule.default.requestCachedBannerAd(config);
}
 ...
 * @returns Promise that resolves when ad is loaded or rejects on failure
 */
function requestGAMBannerAd(requestId, options) {
  if (!requestId || typeof requestId !== 'string') {
    return Promise.reject(new Error('requestId must be a non-empty string'));
  }
  if (!options.unitId) {
