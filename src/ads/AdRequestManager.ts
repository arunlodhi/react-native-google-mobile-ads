...
import type {
  RequestBannerAdParams,
  RequestGAMBannerAdParams,
  AdRequestResponse,
} from '../types/CachedAdProps';
 ...
 */
export async function requestBannerAd(params: RequestBannerAdParams): Promise<AdRequestResponse> {
  const { unitId, size, maxHeight, width, requestOptions } = params;
  ...
  // Validate and stringify request options
  let validatedRequestOptions = '{}';
  if (requestOptions) {
    try {
      const validated = validateAdRequestOptions(requestOptions);
      validatedRequestOptions = JSON.stringify(validated);
    } catch (e) {
  ...
  try {
    const result = await NativeGoogleMobileAdsRequestModule.requestBannerAd(
      unitId,
      size,
      maxHeight,
      width,
 ...
 * or rejects when ad fails to load
 */
export async function requestGAMBannerAd(
  params: RequestGAMBannerAdParams,
): Promise<AdRequestResponse> {
  const { unitId, sizes, maxHeight, width, requestOptions, manualImpressionsEnabled } = params;
  ...
  if (
    ...
    throw new Error(
      "requestGAMBannerAd: 'sizes' expected a valid array of BannerAdSize or custom size strings.",
    );
  }
  ...
  try {
    const result = await NativeGoogleMobileAdsRequestModule.requestGAMBannerAd(
      unitId,
      sizes,
      maxHeight,
      width,
 ...
 */
export function releaseAd(requestId: string): void {
 ...
 */
export function hasAd(requestId: string): boolean {
  if (!requestId || typeof requestId !== 'string') {
    return false;
  }
