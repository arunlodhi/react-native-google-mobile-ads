...
type RNAppNativeModule = NativeModule & {
  eventsNotifyReady: (ready: boolean) => void;
  eventsAddListener: (eventType: string) => void;
  eventsRemoveListener: (eventType: string, all: boolean) => void;
};

const RNAppModule: RNAppNativeModule = NativeModules.RNAppModule;
